export class Renderer {
  ...
  render() {
    ...
    this.render_word();
    this.render_level();
    const percentage = this.stats.render_stats_and_level_bar(this.store.hits_wrong, this.store.hits_correct);
  ...
  render_before_words() {
    ...
    if (this.store.currentIndex != 0) {
      ...
      lines = "<p>" + this.store.currentFile.slice(minLine, maxLine).join('<br>') + "</p>";
    }
  ...
  render_file_progress() {
    ...
    $("#fileProgress").html(`<span>${this.store.currentIndex}/${this.store.currentFile.length} - ${this.store.fileName}</span>`);
  }
